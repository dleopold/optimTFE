...
conservation footprint generated from the `optimTFE` function.
We will use the `solutions.csv` output generated from the algorithm, and then additional
...
#### Bring in datasets
Datasets to create the footprint map include the `solutions.csv` output, planning unit polygon data,
and relevant auxiliary spatial data such existing conservation lands, native habitat, etc.  
All files of interest need to be added to this list.  
**Note:** Currently this tool can only accept spatial polygon vector data in .shp or .gpkg file formats.
We can also map selected planning units of individual species to the map.
...
knitr::include_graphics("./vignette_images/leaflet_map_basic.PNG")
```
...
knitr::include_graphics("./vignette_images/target_species_highlight.PNG")
```
...
knitr::include_graphics("./vignette_images/num_target_species.PNG")
```

#### Create a conservation footprint map with target species and additional spatial data
Use the `add_auxiliary_layers` argument in the `create_interactive_map` function
to add context to the conservation footprint interactive maps. The image below shows
a conservation footprint with a map of existing native habitat.
...
create_interactive_map(
  solution_result_file = solution_file, original_PU_polygons = PU_polygons,
...
knitr::include_graphics("./vignette_images/native_habitat_footprint.PNG")
```
