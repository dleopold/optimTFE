...
#'@param spp_pops_fn input matrix of delineated populations. Row / column names
#'  must match suitability matrix
#'@param spp_occ_fn input matrix of known occurrence locations. Row / column
...
greedyOpt <- function(
  ...
  future::plan(future::multisession, workers = cores) # future backend
  ...
  if (output_csv) {
    arrow::write_csv_arrow(
      data,
      file.path(dir, "output", paste0(output_prefix, ".csv"))
    )
  }
  ...
  if (output_parquet) {
    arrow::write_dataset(
      data,
      file.path(dir, "output", output_prefix),
      format = "parquet"
    )
  }
  ...
  if (return_df) {
    message(crayon::white("Returning results as tibble..."))
    return({
