...
author: "Package authors: Devin Leopold, Christina Leopold and Lucas Berio Fortini"
...
managers identify priority recovery areas, represented by planning units (PUs),
for species recovery planning efforts.
...
We also provide a vignette demonstrating how to visually explore metrics of interest
across generated footprints using an interactive shiny tool.
Another vignette demonstrates how to generate an interactive leaflet application 
for visually exploring species-specific and PU-specific qualities of a conservation
footprint.
...
e.g. sp_01, sp_01, etc. Ensure there is also a .csv with species targets, and
...
The rest of vignette explores the solutions dataset through summary statistics and
inclusion of associated spatial data.
If >100,000 solutions were generated, and depending on computer RAM, subsetting
solutions before creating many summary statistics may reduce run time.

If you did not save the solutions output as an object, read it in now.
...
# sample_sols100 <- read.csv(file.path("./output", "solutions.csv"))
...
# Assign solution_number based on output from .
...
sol_suit_values <- solution_suit_values(solution_number, sample_sols100, file.path("./output", "solutions.meta"))
```
...
sol_selected_spp_suit <- selected_spp_suit(solution_number, sample_sols100, file.path("./output", "solutions.meta"))
```
...
# The all_sols_metrics object created will be used in the optimTFE.explorer tool.
...
# big, so it may be prudent to subset based on footprint unit count or other important metric.
...
# Optional: Export as .csv for running the solutions explorer tool later. This is the
# column structure needed as input.
write.csv(all_sols_metrics, "./output/sample_sols100_metrics.csv")
...
# plot a footprint to see how it compares to other solutions for the percent of the footprint with 'good habitat'
