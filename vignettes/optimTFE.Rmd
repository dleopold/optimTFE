...
PU_metrics <- read_sf("./data/PU_metrics.gpkg")
...
sol_metrics <- all_sols_metrics[2:8]
# Then run min_max_solutions function to generate dateframe with min, max, or both
# for each metric across all solutions
max_sol_calcs <- min_max_solutions(all_sols_metrics, sol_metrics, summary_type = "max")
min_sol_calcs <- min_max_solutions(all_sols_metrics, sol_metrics, summary_type = "min")
# Combine min and max dataframes
min_max_solution_metrics <- bind_rows(max_rows, min_rows)
```
...
```{r Plot distribution of solutions across metrics compared to a single solution, echo=FALSE, fig.show='asis'}
...
plot_my_metric <- plot_solution_metric(53, all_sols_metrics, "good_hab_mean_pct", 0.9)
plot_my_metric
```
