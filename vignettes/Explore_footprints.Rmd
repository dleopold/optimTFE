...
author: "Authors: Devin Leopold, Christina Leopold"
...
library(klippy)
```
## Conservation footprint explorer tool
### Evaluate tradeoffs across footprint metrics
Here we can evaluate tradeoffs across metrics across all conservation footprints
generated. For example, we can understand the amount of a footprint within existing
conservation lands, proportion of a footprint that occurs within fenced areas or native
habitat, etc.
...
for a footprint across two metrics. 
The top-ranked footprint is presented in the map and the dashed red line in the graphs
show where the selection metrics fall across all solutions.
The bottom drop-down menu allows for selection of multiple solutions. Doing so will
add a dashed line for each solution. PU transparency will change on the map according
to the number of selected solutions with overlap.
*Note the check box beneath each metric which allows for selecting descending rank
order.
```{r include metrics, echo=F, out.width = "500px", fig.align="center"}
...
```{r slider bars, echo=F, out.width = "500px", fig.align="center"}
