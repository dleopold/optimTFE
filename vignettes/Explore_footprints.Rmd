...
In this vignette we show how we can evaluate tradeoffs across all conservation footprints
using metrics useful to conservation practitioners.  
For example, we can understand the proportion that a footprint occurs within existing
conservation lands, the proportion of a footprint that occurs within fenced areas
or native habitat, etc. The specific set of criteria considered in the analysis is
project specific, with the examples provided representing a set of criteria identified
as valuable metrics to be used in footprint selection in our particular case study.
...
# (planning unit area, conservation lands, etc.)
...
2) Solution output file, `solutions.csv' generated from running the optimTFE algorithm
...
knitr::include_graphics("./vignette_images/open_webtool.PNG")
```

Next, load the two files into the tool.

```{r load data files, echo=F, out.width = "650px", fig.align="center"}
knitr::include_graphics("./vignette_images/load_data_files.PNG")
```

Now we can see the loaded metrics. Here is an example with two metrics selected. The drop-down
...
show where the selection metrics fall across all generated footprints.  
The bottom drop-down menu allows for selection of multiple footprints. Doing so will
add a dashed line for each footprint. Planning unit transparency will change on the map according
...
order. This is an important feature as some metrics min or max values may be desired (e.g., invaded vs native habitat area per planning unit).
```{r include metrics, echo=F, out.width = "650px", fig.align="center"}
knitr::include_graphics("./vignette_images/metrics_descending_rank_order_option.PNG")
```
...
slider value to reduce the weight of individual metrics.   
...
knitr::include_graphics("./vignette_images/mult_footprints_slider_used.PNG")
```
