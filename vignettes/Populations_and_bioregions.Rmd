...
If you have not first explored the [optimTFE vignette](https://code.usgs.gov/pierc/optimTFE/-/blob/release-candidate/doc/optimTFE.html?ref_type=heads), please explore it before using the populations or bioregions functions.
...
The option to account for existing populations is found in the `populations_file`
function. Each distinct population should be uniquely named; alphanumeric codes
are accepted. Populations can span more than 1 planning unit.
...
A planning unit cannot be assigned to more than one population for a given species.
...
The sub_regions_file function can be used to meet targets across multiple regions.
...
Not all planning units must be assigned to a region, but ensure all columns for the
given planning units are 0s.  

**NOTE:** A planning unit can be assigned only to a single region.
...
The optimTFE algorithm will select a planning unit for both populations in bioregion1, another
PU based on suitability values, and a planning unit from the population in bioregion3
to meet the target of 4.
